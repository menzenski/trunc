...
class RNCSource(object):
    ...
    If a source's name contains only one date, all three ``date`` attributes
    are identical:
    ...
    If a source contains no dates at all, the default date values of ``0``
    are used. This is uncommon in the main (modern) corpus, but happens
    frequently in the historical corpora:
